...
// ============================================================
// DADOS MOCKADOS — TODO: Substituir por chamadas à API/Prisma
...
const Modal = ({ title, children, onClose, width = 560 }) => (
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
    <div style={{ background: C.white, borderRadius: 16, width: "100%", maxWidth: width, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.2)" }}>
...
const AuthScreen = ({ onLogin }) => {
  ...
  const handleLogin = () => {
    ...
    // TODO: POST /api/auth/login → NextAuth.signIn ou supabase.auth.signInWithPassword
    setTimeout(() => { setLoading(false); onLogin({ nome: "Usuário Demo", email: form.email, telefone: "+55 11 99999-0000" }); }, 1000);
  };
  ...
  const handleCadastro = () => {
    ...
    setLoading(true); setErro("");
    // TODO: POST /api/auth/register → bcrypt.hash(senha) + prisma.user.create
    setTimeout(() => { setLoading(false); onLogin({ nome: form.nome, email: form.email, telefone: form.telefone }); }, 1000);
  };
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #f0f4ff 0%, #e8f4fd 100%)", padding: 16 }}>
      <Card style={{ width: "100%", maxWidth: 400, padding: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Btn variant="primary" onClick={() => { setRecuperar(false); }} size="lg" style={{ width: "100%", justifyContent: "center" }}>Enviar link de recuperação</Btn>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "linear-gradient(135deg, #f0f4ff 0%, #e8f4fd 100%)" }}>
      <div style={{ flex: 1, display: "none", background: `linear-gradient(135deg, ${C.brand} 0%, ${C.brandDark} 100%)`, alignItems: "center", justifyContent: "center", padding: 48, flexDirection: "column", gap: 24, "@media(min-width:768px)": { display: "flex" } }} className="auth-left">
        <Logo size={48} white />
        <div style={{ color: C.white, textAlign: "center" }}>
          <h1 style={{ fontSize: 32, fontWeight: 800, margin: "0 0 12px" }}>Controle de Contas</h1>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "center" }}>
          {["Alertas automáticos", "Gráficos detalhados", "Relatórios completos", "WhatsApp integrado"].map(f => (
            <span key={f} style={{ background: "rgba(255,255,255,.15)", color: C.white, padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>✓ {f}</span>
          ))}
        </div>
      </div>
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
        <Card style={{ width: "100%", maxWidth: 440, padding: "32px 36px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Input label="Senha" value={form.senha} onChange={f("senha")} type="password" placeholder="••••••••" required />
            ...
            {tab === "login" && (
              <button onClick={() => setRecuperar(true)} style={{ background: "none", border: "none", color: C.brand, cursor: "pointer", fontSize: 13, fontWeight: 600, textAlign: "center" }}>
                Esqueci minha senha
              </button>
            )}
          </div>
...
const FormConta = ({ initial, onSave, onClose }) => {
  ...
  const submit = () => {
    if (!form.nome || !form.valor || !form.data_vencimento) { setErro("Preencha nome, valor e data de vencimento."); return; }
    // TODO: POST/PUT /api/contas → prisma.conta.create / prisma.conta.update
    const nova = { ...form, id: initial?.id || gerarId(), user_id: "u1", valor: parseFloat(form.valor), created_at: initial?.created_at || new Date().toISOString(), updated_at: new Date().toISOString() };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <Input label="Valor (R$)" value={form.valor} onChange={f("valor")} type="number" placeholder="0,00" required />
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 8 }}>
        ...
        <Btn variant="primary" onClick={submit}>💾 Salvar conta</Btn>
      </div>
    </div>
  );
};
...
// MÓDULO CONTAS
// ============================================================
const ContasModule = ({ contas, setContas, showToast }) => {
  ...
  const handleSave = (conta) => {
    if (contas.find(c => c.id === conta.id)) {
      // TODO: PUT /api/contas/:id → prisma.conta.update
      setContas(prev => prev.map(c => c.id === conta.id ? conta : c));
      showToast("Conta atualizada com sucesso!", "success");
    } else {
      // TODO: POST /api/contas → prisma.conta.create
  ...
  const handleDelete = (id) => {
    // TODO: DELETE /api/contas/:id → prisma.conta.delete
...
const ConfiguracoesModule = ({ user, setUser, showToast }) => {
  ...
  const salvarPerfil = () => {
    // TODO: PUT /api/users/profile → prisma.user.update + bcrypt.hash(novaSenha)
  ...
  return (
    <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 20, maxWidth: 700 }}>
      ...
      <Card style={{ padding: 24 }}>
        ...
        <div style={{ marginTop: 16 }}>
          <Btn variant="primary" onClick={() => showToast("Preferências salvas!", "success")}>💾 Salvar preferências</Btn>
        </div>
      </Card>
...
export default function App() {
  ...
  const [contas, setContas] = useState(CONTAS_MOCK.map(c => ({ ...c, status: calcularStatus(c) })));
  ...
  const handleSaveConta = (conta) => {
    if (contas.find(c => c.id === conta.id)) setContas(prev => prev.map(c => c.id === conta.id ? conta : c));
    else setContas(prev => [...prev, conta]);
  ...
  const titulos = { dashboard: "Dashboard", contas: "Minhas Contas", alertas: "Central de Alertas", relatorios: "Relatórios", whatsapp: "Integração WhatsApp", configuracoes: "Configurações" };

  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", background: C.gray50, color: C.gray900 }}>
      <Sidebar active={paginaAtiva} onNav={setPaginaAtiva} collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed(p => !p)} user={user} onLogout={() => setUser(null)} />

      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
        <Header titulo={titulos[paginaAtiva]} alertas={alertasCount} />

        <main style={{ flex: 1, overflowY: "auto" }}>
          {modalNovaConta && <Modal title="Nova Conta" onClose={() => setModalNovaConta(false)} width={620}>
          ...
          {paginaAtiva === "contas" && <ContasModule contas={contas} setContas={setContas} showToast={showToast} />}
