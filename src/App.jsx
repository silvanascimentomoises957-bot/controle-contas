...
const Dashboard = ({ contas, onNovaConta }) => {
  ...
  // Detecta qual mês/ano tem mais contas para iniciar a exibição nele
  const detectarMesInicial = () => {
    if (!contas || contas.length === 0) return { mes: mesAtual, ano: anoAtual };
    // Conta ocorrências por mês/ano
    const contagem = {};
    contas.forEach(c => {
    ...
    // Prefere o mês atual se tiver contas; senão pega o com mais contas
    const keyAtual = `${anoAtual}-${mesAtual}`;
    if (contagem[keyAtual]) return { mes: mesAtual, ano: anoAtual };
    const melhor = Object.entries(contagem).sort((a, b) => b[1] - a[1])[0];
    if (melhor) {
      const [ano, mes] = melhor[0].split("-").map(Number);
      return { mes, ano };
    }
    return { mes: mesAtual, ano: anoAtual };
  };

  const inicial = detectarMesInicial();
  const [mesSel, setMesSel] = useState(inicial.mes);
  const [anoSel, setAnoSel] = useState(inicial.ano);

  // Atualiza mês exibido quando contas mudam (ex: ao carregar do Supabase)
  useEffect(() => {
    if (contas && contas.length > 0) {
      const { mes, ano } = detectarMesInicial();
      setMesSel(mes);
      setAnoSel(ano);
    }
  }, [contas.length]);
  ...
  const barData = mesesLabel.map((m, i) => {
    const cs = contas.filter(c => {
      if (!c.data_vencimento) return false;
      const d = new Date(c.data_vencimento + "T00:00:00");
      return d.getMonth() === i && d.getFullYear() === anoSel;
    });
  ...
  return (
    <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* Seletor de mês */}
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={irMesAnterior} style={{ background: C.gray100, border: `1px solid ${C.gray200}`, borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", color: C.gray700 }}>‹</button>
        <span style={{ fontWeight: 700, fontSize: 16, color: C.gray900, minWidth: 140, textAlign: "center" }}>
          {mesesLabel[mesSel - 1]} / {anoSel}
        </span>
        <button onClick={irMesProximo} style={{ background: C.gray100, border: `1px solid ${C.gray200}`, borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", color: C.gray700 }}>›</button>
        {(mesSel !== mesAtual || anoSel !== anoAtual) && (
          <button onClick={() => { setMesSel(mesAtual); setAnoSel(anoAtual); }} style={{ background: "none", border: `1px solid ${C.brand}`, color: C.brand, borderRadius: 8, padding: "4px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Mês atual</button>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <Card style={{ padding: 20 }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: C.gray900 }}>Por Categoria ({mesesLabel[mesSel - 1]})</h3>
        ...
        <Card style={{ padding: 20 }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: C.gray900 }}>Despesas por Mês ({anoSel})</h3>
