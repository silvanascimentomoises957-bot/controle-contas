...
const ConfiguracoesModule = ({ user, setUser, showToast }) => {
  ...
  const [prefs, setPrefs] = useState(() => {
    try { return JSON.parse(localStorage.getItem("cf_prefs")) || { moeda: "BRL", diaAlertaPadrao: "7", categorias: [...CATEGORIAS_DEFAULT] }; }
    catch { return { moeda: "BRL", diaAlertaPadrao: "7", categorias: [...CATEGORIAS_DEFAULT] }; }
  });
  ...
  const salvarPreferencias = () => {
    // Salva moeda e preferências no localStorage
    localStorage.setItem("cf_prefs", JSON.stringify(prefs));
    showToast("Preferências salvas! Recarregue a página para aplicar a moeda.", "success");
  };
  ...
  return (
    <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 20, maxWidth: 700 }}>
      ...
      <Card style={{ padding: 24 }}>
        ...
        <div style={{ marginTop: 16 }}>
          <Btn variant="primary" onClick={salvarPreferencias}>💾 Salvar preferências</Btn>
        </div>
      </Card>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setCarregando(true);
    sb.getContas(user.token)
      .then(data => {
        if (Array.isArray(data)) {
          // Recalcula status para detectar vencimentos overnight
      ...
      .catch(() => showToast("Erro ao carregar contas. Verifique sua conexão.", "error"))
  ...
  const handleSaveConta = async (conta) => {
    const payload = { ...conta };
    delete payload.id; // remove id para insert (Supabase gera)

    const existe = contas.find(c => c.id === conta.id);
    if (existe) {
      // Editar conta existente
      const res = await sb.updateConta(user.token, conta.id, {
        nome: conta.nome, descricao: conta.descricao, tipo: conta.tipo,
        categoria: conta.categoria, valor: conta.valor,
        data_vencimento: conta.data_vencimento || null,
        data_pagamento: conta.data_pagamento || null,
        status: calcularStatus(conta), recorrente: conta.recorrente,
        frequencia: conta.frequencia, forma_pagamento: conta.forma_pagamento,
        observacoes: conta.observacoes, updated_at: new Date().toISOString(),
      });
      ...
      // Criar nova conta
      const res = await sb.insertConta(user.token, {
        id: conta.id, user_id: user.id,
        nome: conta.nome, descricao: conta.descricao, tipo: conta.tipo,
        categoria: conta.categoria, valor: conta.valor,
        data_vencimento: conta.data_vencimento || null,
        data_pagamento: conta.data_pagamento || null,
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
        ...
        <div style={{ background: "#f0fdf4", borderBottom: "1px solid #bbf7d0", padding: "8px 24px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <Btn variant="ghost" size="sm" style={{ marginLeft: "auto", fontSize: 12 }}
            onClick={() => { setCarregando(true); sb.getContas(user.token).then(d => { if (Array.isArray(d)) setContas(d.map(c => ({ ...c, status: calcularStatus(c) }))); }).finally(() => setCarregando(false)); }}>
            🔄 Atualizar
          </Btn>
        </div>
